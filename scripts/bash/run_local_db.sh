...
#echo "DOCKER_PORT_FORWARD: $DOCKER_PORT_FORWARD"
#echo "clean: $clean"
#echo "purge: $purge"
...
#echo "directory_exists: $(if directory_exists; then echo "true"; else echo "false"; fi)"
...
# Start up
if port_is_in_use "${DOCKER_PORT_FORWARD}"; then
    echo "Port ${DOCKER_PORT_FORWARD} is already in use. Exiting."
    exit 1
fi
...
if docker_container_running "${DOCKER_DB_CONTAINER_NAME}"; then
  echo "Container '${DOCKER_DB_CONTAINER_NAME}' is now running on port ${DOCKER_PORT_FORWARD}"
else
